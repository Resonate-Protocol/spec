...
Whether an implemented method is enabled is local client configuration, changed only through a manufacturer-defined action. A disabled method is omitted from [`supported_pair_methods`](messaging.md#client--server-clienthello) and its PSK, if any, is excluded from the handshake candidate set (see [Pre-Shared Key](connection.md#pre-shared-key)).
