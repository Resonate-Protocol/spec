...
The `visualizer` object in [`stream/request-format`](../../messaging.md#client--server-streamrequest-format) has this structure:
...
Response when a `visualizer` stream is active: [`stream/start`](../../messaging.md#server--client-streamstart) with the new visualizer configuration.
