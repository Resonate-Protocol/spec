...
`current_time` and `metadata.timestamp` must both be in the server clock domain, in microseconds. Obtain `current_time` by inverting the [time filter](../../messaging.md#clock-synchronization)'s server-to-local mapping for the current local clock reading, including both offset and drift.
