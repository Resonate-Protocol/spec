...
The `announcement` object in [`client/state`](../../messaging.md#client--server-clientstate) has this structure:
...
Unlike media audio, announcement chunks are not sync-critical and MUST NOT be dropped for being late: the client SHOULD begin output at the first chunk's timestamp (translated via [clock synchronization](../../messaging.md#clock-synchronization)), but if that time has already passed on arrival, it begins as soon as possible, and mid-stream chunks are played continuously in order. The [player sync accuracy rules](../player/v1.md#playback-synchronization) do not apply to announcement output.
