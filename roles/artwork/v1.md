...
Binary messages SHOULD be rejected if there is no active stream or the client is not [`available`](../../messaging.md#client--server-clientstate). A rejected announce still starts its transfer, whose parts are likewise rejected.
