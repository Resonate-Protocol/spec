...
The server MUST NOT send a player command to a client before that client has sent a [`client/state`](../../messaging.md#client--server-clientstate) containing the `player` object, which is where the client advertises `supported_commands`.
