...
When the server removes `source` from [`active_roles`](../../messaging.md#server--client-serveractivate), the client sends `stream/client-end` and stops sending chunks.

A source with an open input stream that becomes [`available: false`](../../messaging.md#external-source-handling) sends `stream/client-end` before it reports `available: false` in `client/state`; the server treats the transition as an implicit `stop`.
...
Binary messages SHOULD be rejected by the server if there is no open input stream (i.e., received before a `stream/client-start` or after a `stream/client-end`) or the client is not [`available`](../../messaging.md#client--server-clientstate).
