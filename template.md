...
- **Sendspin Pairing Code** - a decimal-digit value used in code-based [pairing](pairing.md#pairing) methods. The static-pairing-code method uses a fixed 8-digit value; the dynamic-pairing-code method uses a per-session generated value of variable length (see [Dynamic Pairing Code Flow](pairing.md#dynamic-pairing-code-flow)).
