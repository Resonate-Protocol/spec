...
1. Calculate the delta: `delta = requested_volume - current_group_volume` (where current group volume is the average of the volumes of the players in the group that support the `volume` command)
2. Apply the delta to each of those players' volumes
...
The loop only computes the final per-player volumes; once it completes, the server sends each of those players a single volume command — intermediate values are never sent.
