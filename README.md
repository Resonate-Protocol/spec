...
- **Track transitions** (a track ends and the next begins naturally): no stream commands should be sent, except an in-place `stream/start` when the stream configuration changes. The stream continues uninterrupted to support gapless playback and server-inserted crossfade.
