...
- `supported_pair_methods`: object[] - pairing methods this client offers, each described by a [pair-method descriptor](#client--server-clienthello-pair-method-descriptor). Never empty: every client implements at least the Pairing PSK method (see [Pairing](#pairing)).
...
2. Before sending [`client/pair-finalize`](#client--server-clientpair-finalize), the client MUST verify that the connection's matched PSK is the Pairing PSK (the receiving side of the `selected_pair_method` invariant in [`server/activate`](#server--client-serveractivate)); on mismatch it aborts with [`pair/abort`](#client--server-pairabort) reason `method_not_supported`.
...
- **Increment.** The counter for a method increments on each inner-authentication failure observed in that method's flow: the client's own verification of `server_kc` fails, or [`pair/abort`](#client--server-pairabort) with reason `pin_mismatch` arrives. No other event increments the counter.
