...
**Pairing Token.** A server needs both the [Sendspin Pairing PSK](#definitions) and the client's static public key to select and verify the client's Noise identity. The two are distributed together inside of a version-0 [pairing token](#pairing-token):
