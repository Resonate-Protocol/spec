...
- `command`: 'play' | 'pause' | 'stop' | 'next' | 'previous' | 'seek' | 'volume' | 'mute' | `repeat_off` | `repeat_one` | `repeat_all` | `clear_playlist` - must be one of the values listed in `group/update` field `supported_commands`
