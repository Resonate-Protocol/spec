...
**Server behavior:**
- Compute per-player send-ahead as `max(required_lead_time_ms, min_buffer_ms) + static_delay_ms`. This single floor satisfies both the startup lead (codec/DAC warmup before the first chunk) and the ongoing jitter buffer, since for live sources the buffer cannot grow after playback begins.
