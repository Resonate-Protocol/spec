...
- If the connection's `trust_level` is `'none'` (e.g., a pairing handshake), ignore the message and continue unchanged.
...
Starts the PIN-pairing [attempt](#entering-and-leaving-pairing). In static PIN, sent after the operator gesture opens the [pairing window](#pairing-window). In dynamic PIN, sent immediately after [`server/activate`](#server--client-serveractivate). The server must not send [`server/pair-auth`](#server--client-serverpair-auth) (static PIN) or [`server/pair-init`](#server--client-serverpair-init) (dynamic PIN) before receiving this message.
...
This section describes messages specific to clients with the `announcement` role, which handles short, client-specific audio clips (voice-assistant responses, doorbell chimes, alerts, notifications). An announcement is delivered as its own stream, independent of the `player` role's media stream: it can play while media is playing (mixed over it, optionally ducking the media) or while nothing is playing at all. Media playback is never paused or interrupted by an announcement, so a grouped player's media timeline stays untouched while it plays an announcement.

Announcements are strictly per-client: an announcement stream is addressed to a single client, and no cross-client synchronization is defined for it. A multi-room announcement is simply multiple independent per-client announcement streams.
...
**Note:** The announcement stream is decoded on a second, concurrent pipeline next to the media stream. Clients SHOULD advertise inexpensive formats here (e.g., mono, 16-bit, modest sample rates) so that decoding and mixing stay cheap on constrained hardware. This list is independent of the player's `supported_formats`, and `buffer_capacity` is independent of the player's buffer.

**Note:** Servers must support all audio codecs: 'opus', 'flac', and 'pcm'.

### Client → Server: `client/state` announcement object

The `announcement` object in [`client/state`](#client--server-clientstate) has this structure:

Informs the server of announcement-specific state changes. Only for clients with the `announcement` role. All fields are optional; a minimal client MAY never send this object. The server derives announcement completion from the stream timeline regardless (see [Server behavior for announcements](#server-behavior-for-announcements)), so this state is informational.

- `announcement`: object
  - `state?`: 'playing' | 'idle' - whether announcement audio is currently being output. SHOULD be sent on transitions (output started; output finished)
...
Re-sending `stream/start` with an `announcement` object while an announcement stream is active updates the stream configuration without clearing buffers (e.g., to change the duck level mid-announcement). A new `media_duck_db` target ramps from the current gain; it never jumps. To replace the announcement audio itself, the server MUST first send [`stream/clear`](#server--client-streamclear) listing the `announcement` role, then continue with chunks of the new clip (optionally after a new `stream/start`).

### Server → Client: `stream/clear` announcement

When [`stream/clear`](#server--client-streamclear) explicitly lists the `announcement` role, clients drop all buffered announcement audio and continue with announcement chunks received after this message. The announcement stream stays active and the ducking state is kept. A `stream/clear` that omits the `roles` field does NOT affect the announcement stream: it means a media seek and clears the player and visualizer streams only.
...
Unlike media audio, announcement chunks are not sync-critical and MUST NOT be dropped for being late: the client SHOULD begin output at the first chunk's timestamp (translated via [clock synchronization](#clock-synchronization)), but if that time has already passed on arrival, it begins as soon as possible, and mid-stream chunks are played continuously in order. The [player sync accuracy rules](#playback-synchronization) do not apply to announcement output.
...
Clients that cannot apply a fractional gain reduction MAY implement any non-zero `media_duck_db` as full attenuation of the media signal (duck to silence). Ducking is a no-op while no media is playing locally, and the whole role works without a `player` role active.
...
- Announcement authority equals playback authority: any connection allowed to stream media to the client may announce. There is no negative acknowledgement; a client MAY silently ignore an announcement stream it cannot currently service.
